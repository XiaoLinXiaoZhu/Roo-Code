...
describe("shortId consistency between addNode and recalculateShortIds", () => {
	...
	test("root nodes: addNode after reparent produces consistent shortId", () => {
		...
		const s = tree.addNode({ type: "subgoal", content: "Sub root", parentId: null, taskId: "t1" })

		expect(g1.node.shortId).toBe("G1")
		expect(s.node.shortId).toBe("S2") // Global counter: 2

		// reparent S2 under G1 → recalculateShortIds syncs rootMaxChildIndex to 1
		tree.reparentNode("S2", "G1", "t1")
	...
	test("child nodes: addNode after reparent produces sequential shortId without gaps", () => {
		...
		tree.addNode({ type: "subgoal", content: "Sub 3", parentId: "G1", taskId: "t1" })

		// G1 children: S1.1, S1.2, S1.3

		// Reparent S1.2 to root → recalculate compacts to S1.1, S1.2 and syncs maxChildIndex=2
		tree.reparentNode("S1.2", null, "t1")

		// Add new child — should get S1.3 (not S1.4)
		const newChild = tree.addNode({ type: "subgoal", content: "Sub 4", parentId: "G1", taskId: "t1" })
		expect(newChild.node.shortId).toBe("S1.3")

		// Children should be sequential: S1.1, S1.2, S1.3
		const childIds = tree.getChildren("G1").map((n) => n.shortId)
		expect(childIds).toEqual(["S1.1", "S1.2", "S1.3"])
	})
	...
	test("reparent + addNode + reparent: shortIds remain stable", () => {
		...
		tree.addNode({ type: "subgoal", content: "Sub A", parentId: "G1", taskId: "t1" })

		// Reparent Sub from G1 to G2
		tree.reparentNode("S1.1", "G2", "t1")

		// Add new child to G1
		const newChild = tree.addNode({ type: "subgoal", content: "Sub B", parentId: "G1", taskId: "t1" })
		expect(newChild.node.shortId).toBe("S1.1") // G1 had 0 children after reparent, maxChildIndex synced to 0, so next is 1

		// Reparent Sub A back to G1
		tree.reparentNode("S2.1", "G1", "t1")
		...
		expect(g1Children[0]).toBe("S1.1")
		expect(g1Children[1]).toBe("S1.2")
	})
